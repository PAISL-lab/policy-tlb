...
def write_report(result_dir: Path, e2e_summary: list[dict], latency_summary: list[dict]) -> None:
    ...
    lines.extend(
        [
            ...
            "",
            "- The naive eBPF LSM baseline is independent code and intentionally does not use MCPGuard's L1 cache, L2 fast path, per-process resource-id decision cache, tail-call pipeline, or epoch reuse.",
            "- The naive eBPF LSM baseline still evaluates command-prefix, path-prefix, resource-id, and socket-port policy classes with cacheless rule/map lookup on every observed hook event.",
