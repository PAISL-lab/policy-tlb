...
static int is_default_policy_dir(const char *policy_dir)
{
	return !policy_dir ||
	       strcmp(policy_dir, "policies") == 0 ||
	       strcmp(policy_dir, "./policies") == 0;
}

static int graphical_session_available(void)
{
	...
	if (sudo_uid && sudo_uid[0]) {
		snprintf(runtime_dir, sizeof(runtime_dir), "/run/user/%s", sudo_uid);
		if (access(runtime_dir, X_OK) == 0)
			return 1;
	}
...
static int gui_entrypoint_available(void)
{
	return access("gui/run_gui.py", R_OK) == 0;
}

static int should_launch_gui(const struct cli_options *options)
{
	if (!options)
		return 0;
	if (options->gui_mode == 0)
		return 0;
	if (options->gui_mode < 0 && env_is_disabled("MCP_GUARD_AUTO_GUI"))
		return 0;
	if (options->gui_mode < 0 && !is_default_policy_dir(options->policy_dir))
		return 0;
	if (!graphical_session_available())
		return 0;
	if (!gui_entrypoint_available())
		return 0;
	return 1;
}

static void configure_gui_user_env(uid_t uid)
{
	...
	snprintf(runtime_dir, sizeof(runtime_dir), "/run/user/%u", uid);
	if (access(runtime_dir, X_OK) == 0) {
		setenv("XDG_RUNTIME_DIR", runtime_dir, 1);
		snprintf(wayland_socket, sizeof(wayland_socket), "%s/wayland-0",
			 runtime_dir);
		if (!getenv("WAYLAND_DISPLAY") && access(wayland_socket, F_OK) == 0)
...
static int drop_to_sudo_user(void)
{
	...
	errno = 0;
	uid_value = strtoul(uid_env, &end, 10);
	if (errno || end == uid_env || *end)
		return -EINVAL;
	errno = 0;
	gid_value = strtoul(gid_env, &end, 10);
	if (errno || end == gid_env || *end)
		return -EINVAL;

	pw = getpwuid((uid_t)uid_value);
	if (pw)
		initgroups(pw->pw_name, (gid_t)gid_value);
	if (setgid((gid_t)gid_value) != 0)
		return -errno;
	if (setuid((uid_t)uid_value) != 0)
		return -errno;

	configure_gui_user_env((uid_t)uid_value);
	return 0;
}

static const char *gui_python_path(void)
{
	if (access("gui/.venv/bin/python3", X_OK) == 0)
		return "gui/.venv/bin/python3";
	if (access("gui/.venv/bin/python", X_OK) == 0)
		return "gui/.venv/bin/python";
	return "python3";
}

static pid_t launch_gui_if_needed(const struct cli_options *options)
{
	const char *python;
	pid_t pid;

	if (!should_launch_gui(options))
		return 0;

	python = gui_python_path();
	pid = fork();
	if (pid < 0) {
		fprintf(stderr, "warning: failed to launch GUI: %s\n", strerror(errno));
		return 0;
	}
	if (pid == 0) {
		int log_fd;
		...
		setsid();
		log_fd = open("/tmp/mcp-guard-gui.log", O_CREAT | O_WRONLY | O_APPEND, 0644);
		if (log_fd >= 0) {
			fchmod(log_fd, 0644);
		...
		if (drop_to_sudo_user() != 0)
			_exit(127);
		execlp(python, python, "gui/run_gui.py", "--socket",
		       MCP_GUARD_SOCKET_PATH, (char *)NULL);
		_exit(127);
	}

	printf("GUI auto-launch requested, pid=%d\n", pid);
	return pid;
}

static void stop_gui_process(pid_t pid)
{
	int status;
...
int main(int argc, char **argv)
{
	...
	if (err) {
		fprintf(stderr,
			"usage: %s [policy_dir] [--metrics-interval seconds] [--gui|--no-gui]\n",
			argv[0]);
		goto out;
	}
	...
	gui_pid = launch_gui_if_needed(&options);
